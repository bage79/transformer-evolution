...
from bert.model import BertTrainMovie
from config import Config, get_model_filename, is_mac_or_pycharm
...
def eval_epoch(config, rank, model, data_loader):
    ...
    with tqdm(data_loader, desc=f"eval({rank})", position=2, leave=False) as pbar:
        for value in pbar:
...
def train_epoch(config, rank, model, criterion_cls, optimizer, scheduler, train_loader):
    """ 모델 epoch 학습 """
    losses = []
    model.train()

    for value in tqdm(train_loader, desc=f"train({rank})", position=1, leave=False):
        labels, inputs, segments = map(lambda v: v.to(config.device), value)
...
def train_model(rank, world_size, args):
    ...
    train_model = BertTrainMovie(config)
    if os.path.isfile(args.save):
        try:
            best_epoch, best_loss, best_score = train_model.load(args.save)
            print(f"rank: {rank} load state dict from: {os.path.basename(args.save)}")
        except:
        ...
        try:
            ...
            print(f"rank: {rank} load pretrain from: {os.path.basename(args.pretrain_save)}, epoch={epoch}, loss={loss}")
        except:
    ...
    train_loader: DataLoader = data.build_data_loader(vocab, args.train, args, data_type='train', shuffle=True)
    test_loader: DataLoader = data.build_data_loader(vocab, args.test, args, data_type='test', shuffle=False)

    t_total = len(train_loader) * args.epoch
    no_decay = ['bias', 'LayerNorm.weight']
    optimizer_grouped_parameters = [
        {'params': [p for n, p in train_model.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay},
    ...
    with trange(args.epoch, desc="Epoch", position=0) as pbar:
        ...
        for step in pbar:
            ...
            loss = train_epoch(config, rank, train_model, criterion_cls, optimizer, scheduler, train_loader)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--epoch", default=20 if not is_mac_or_pycharm() else 5, type=int, required=False,
                        help="epoch")
    parser.add_argument("--batch", default=512 if not is_mac_or_pycharm() else 3, type=int, required=False,
                        help="batch")
    ...
    parser.add_argument('--project', default='bert-train', type=str, required=False, help='project name for wandb')
    args = parser.parse_args()
    args.save = os.path.abspath(os.path.join(os.getcwd(), "save", f'{get_model_filename(args)}.pth'))
